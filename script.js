// script.js - VERSÃO COMPLETAMENTE CORRIGIDA
import { supabase, testarConexao } from './supabase-config.js';
import { 
  buscarProdutos, 
  criarPedido, 
  buscarConfiguracoes,
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    console.log('🔗 Conexão testada:', conectado);
    
    if (!conectado) {
      Toastify({
        text: '⚠️ Erro ao conectar com o banco de dados!',
        duration: 5000,
...
async function carregarProdutos() {
  try {
    ...
    console.log(`✅ ${produtosLoja.length} produtos carregados`);
...
function renderizarProdutos(lista) {
  ...
  if (!lista || lista.length === 0) {
    container.innerHTML = `
      <div class="col-span-full text-center py-12 text-slate-400">
        ...
        <p class="text-sm">Adicione produtos no painel administrativo.</p>
      </div>
    `;
    return;
  }
  ...
  container.innerHTML = lista.map((p) => {
    ...
    const imgSrc = (p.image && !p.image.includes('via.placeholder.com')) ? p.image : placeholderImg;
    ...
    return `
      <div class="product-card bg-white rounded-[2.5rem] p-3 flex flex-col relative group transition-all duration-500 hover:shadow-2xl hover:shadow-primary/10 ${isEsgotado ? "opacity-60" : ""}">
        ...
        <div class="px-3 pb-2">
          <p class="text-[9px] uppercase tracking-[0.2em] text-primary font-bold mb-1 opacity-70">${p.category || 'Geral'}</p>
          <h3 class="text-dark font-extrabold text-sm mb-1 truncate leading-tight">${p.name || 'Sem nome'}</h3>
          ...
          <div class="flex justify-between items-center border-t border-slate-50 pt-3">
            <div class="flex flex-col">
              ...
              <span class="text-base font-black text-dark">R$ ${(p.price || 0).toFixed(2).replace(".", ",")}</span>
            </div>
...
function renderizarDestaques() {
  ...
  if (destaques.length === 0) {
    destaqueContainer.innerHTML = `
      <div class="swiper-slide text-center py-8 text-slate-400">
    ...
    <div class="swiper-slide py-10">
      <div class="destaque-card-premium group relative overflow-hidden rounded-[3rem] p-6">
        ...
        <div class="text-center relative z-10">
          <p class="text-[9px] uppercase tracking-[0.3em] text-primary/60 font-bold mb-2">${p.category || 'Geral'}</p>
          <h3 class="font-extrabold text-base text-dark mb-4 leading-tight">${p.name || 'Sem nome'}</h3>
          <div class="mb-6">
            <span class="text-2xl font-black text-dark tracking-tighter">R$ ${(p.price || 0).toFixed(2).replace(".", ",")}</span>
          </div>
...
function renderizarCategorias() {
  ...
  const categorias = [...new Set(produtosLoja.map(p => p.category).filter(c => c))];
...
function setupEventListeners() {
  ...
  if (closeModalBtn) {
    closeModalBtn.onclick = () => cartModal.classList.replace("flex", "hidden");
  }
  ...
  if (mobileOverlay) {
    mobileOverlay.onclick = window.toggleMenu;
  }
  
  if (closeMobileMenu) {
    closeMobileMenu.onclick = window.toggleMenu;
  }
...
window.addEventListener('beforeinstallprompt', (e) => {
  ...
  if (window.innerWidth < 768 && !localStorage.getItem('bannerClosed')) {
    setTimeout(() => {
      if (installBanner) installBanner.classList.remove('translate-y-[150%]');
    }, 3000);
  }
});
...
if (document.getElementById('btn-install-now')) {
  document.getElementById('btn-install-now').addEventListener('click', async () => {
    if (deferredPrompt) {
      ...
      deferredPrompt = null;
      if (installBanner) installBanner.classList.add('translate-y-[150%]');
    } else {
